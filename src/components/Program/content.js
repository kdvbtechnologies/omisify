export const accordionData = [
  {
    title: "Programme 1",
    program: "Programme d'Affiliation : Devenir Partenaire Omisify",
    programLink: "Lien d'Affiliation : https://omisify.com",
    programDesc: `Omisify est une plateforme qui proprose des programmes d'affiliation. Vous pourrez augmenter vos revenus en devenant Partenaire Omisify. Omisify est gratuit et rémunère toujours ses partenaires quelque soit le pays dans lequel vous vous trouver et quelque soit la situation de votre pays. Avec Omisify vous pourrez retirer votre rémunération à tout moment que vous le souhaiterez. Gagnez de l'argent avec Omisify !`,
    points: "Nombre de points : 2 points (BONUS DE BIENVENUE)",
    consignes: "Consignes :",
    comment:
      "Commentaire : Publier un commentaire dans lequel vous insérez votre lien d'affiliation",
    commentG: "Gain : Chaque commentaire publier est équivaut à 1 point",
    message:
      "Envoyer un message dans lequel vous insérer votre lien d'affiliation",
    messageG: "Gain : Chaque message envoyé est équivaut à 1 point",
    publication:
      "Publier une publication dans lequel vous insérer votre lien d'affiliation",
    publicationG:
      "Gain : Chaque publication est équivaut à 1 point et chaque like sur une publication est équivaut à 0.0001€",
  },
  {
    title: "Programme 2",
    program: "Programme d'Affiliation : Création de site internet",
    programLink: "Lien d'Affiliation : https://carilizo.com",
    programDesc: `Carilizo est une entreprise de création de site internet. Faites créer votre site internet par Carilizo au prix de 10€ seulement. Le payement se fera après la livraison !`,
    points: "Nombre de points : 1 point",
    consignes: "Consignes :",
    comment:
      "Commentaire : Publier un commentaire dans lequel vous insérez votre lien d'affiliation",
    commentG: "Gain : Chaque commentaire publier est équivaut à 1 point",
    message:
      "Envoyer un message dans lequel vous insérer votre lien d'affiliation",
    messageG: "Gain : Chaque message envoyé est équivaut à 1 point",
    publication:
      "Publier une publication dans lequel vous insérer votre lien d'affiliation",
    publicationG:
      "Gain : Chaque publication est équivaut à 1 point et chaque like sur une publication est équivaut à 0.0001€",
  },
  /*
  {
    title: "Programme 3",
    program: "a",
    programLink: "Lien d'Affiliation : ",
    programDesc: `a`,
    points: "Nombre de points : 1 point",
    consignes: "Consignes :",
    comment:
      "Commentaire : Publier un commentaire dans lequel vous insérez votre lien d'affiliation",
    commentG: "Gain : Chaque commentaire publier est équivaut à 1 point",
    message:
      "Envoyer un message dans lequel vous insérer votre lien d'affiliation",
    messageG: "Gain : Chaque message envoyé est équivaut à 1 point",
    publication:
      "Publier une publication dans lequel vous insérer votre lien d'affiliation",
    publicationG:
      "Gain : Chaque publication est équivaut à 1 point et chaque like sur une publication est équivaut à 0.0001€",
  },*/
];
